    ...
    <GlassCard title="Pollutant Composition">
              ...
              return (
                  <div key={item.label} className="flex flex-col items-center">
                      <div className="relative w-14 h-14 flex items-center justify-center">
                          ...
                          <span className={`absolute text-[10px] font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                             {Math.round(item.val)}
                          </span>
                      </div>
                      <span className={`text-[10px] font-bold mt-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{item.label}</span>
                  </div>
              );
          })}
       </div>
       ...
       <div className="mt-6 pt-4 border-t border-slate-700/50">
           <div className="flex justify-between items-center mb-2">
               <span className="text-[10px] text-slate-500 uppercase font-bold">6H Trend</span>
           ...
           <div className="h-8 flex items-end gap-1">
               {[...Array(20)].map((_, i) => (
                   <div 
                     ...
                     style={{ height: `${20 + Math.random() * 60}%`, opacity: (i+1)/20 }}
                   ></div>
               ))}
           </div>
       </div>

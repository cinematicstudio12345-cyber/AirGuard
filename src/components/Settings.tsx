...
import { auth, updateUserTheme } from '../services/firebase';
...
const Settings: React.FC<SettingsProps> = ({ user, onLogout }) => {
  ...
  const handleThemeToggle = async () => {
    toggleTheme();
    if (auth.currentUser) {
      await updateUserTheme(auth.currentUser.uid, !isDark);
    }
  };
    ...
    <div className="p-4 max-w-lg mx-auto pb-24 space-y-6">
      ...
      <div className="space-y-3">
         ...
         <button className={`w-full p-4 rounded-xl flex items-center justify-between border transition-colors ${isDark ? 'bg-slate-900/50 border-slate-800 hover:bg-slate-800' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
            ...
            <span className="text-xs text-slate-500">WeatherAPI + Firestore</span>
         </button>
      </div>
      ...
      <div className="pt-8">
         ...
         <p className="text-center text-[10px] text-slate-600 mt-4 uppercase tracking-widest">AirGuard Pro v2.5.1</p>
      </div>
    </div>
  );
};
